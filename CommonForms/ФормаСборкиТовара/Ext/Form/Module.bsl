...
Процедура ШК_СборкаЗаказ(пШК)
	...
	Если Не ЗначениеЗаполнено(ДанныеТовара.Идентификатор) Тогда
		ПоказатьПредупреждение(, ОбщегоНазначения_Сервер.ПолучитьТекстОшибки("НеопределённыйТовар"));
		Возврат;
	КонецЕсли;
...
Функция ПолучитьДанныеТовараПоШтрихкоду(пШК)
	...
	#Если МобильноеПриложениеКлиент Тогда
		Если Не ЗначениеЗаполнено(ДанныеТовара.Идентификатор) Тогда
			СредстваМультимедиа.ВоспроизвестиАудио(ОбщегоНазначения_Сервер.ПолучитьДвоичныеДанныеЗвука("Звук_НеправильныйШтрихкод"));
